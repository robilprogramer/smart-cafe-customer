...
export default function AdminDashboard() {
  const stats = [
    { label: 'Total Pengguna', value: '1,234', change: '+12%', color: 'blue' },
              ...
              {[1, 2, 3, 4].map((item) => (
                <div key={item} className="flex items-center gap-4 pb-4 border-b border-gray-100 last:border-0">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-800">User melakukan aksi #{item}</p>
                    <p className="text-xs text-gray-500">{item} menit yang lalu</p>
                  </div>
                </div>
              ))}
            </div>
